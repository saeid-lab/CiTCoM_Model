...
echo "##### Adding magnesium ions to the simulation box #####"
sleep 5
# Calculate magnesium ions (0.02 M concentration)
c_mg=$(awk -v n="$n_waters" 'BEGIN { printf "%.0f", (n * 0.02) / 55.5 }')
c_mg_cl=$((c_mg * 2))

echo "Number of Mg ions to add: $c_mg"
echo "Number of additional Cl ions for Mg: $c_mg_cl"

# Generate TPR file from the KCl system
gmx grompp -f min.mdp -c "${pdbid}_sol_kcl.gro" -r "${pdbid}_sol_kcl.gro" \
    -p topol.top -o genion_input_2.tpr -maxwarn 1 \
    >> log_autogmx.log 2>&1

# Add Mg2+ and compensating Cl- ions
echo 'SOL' | gmx genion -s genion_input_2.tpr -p topol.top -o "${pdbid}_sol_kcl_mgcl.gro" \
    -pname MG -pq 2 -np "$c_mg" \
    -nname CL -nq -1 -nn "$c_mg_cl" \
    >> log_autogmx.log 2>&1
    
    
echo "##### Magnesium ion(s) were added to the simulation box #####"
sleep 5

echo "##### Creating minimzation input file #####"
sleep 3
gmx grompp -f min.mdp -c "${pdbid}_sol_kcl_mgcl.gro" -p topol.top -o input_min.tpr -maxwarn 1 -r "${pdbid}_sol_kcl_mgcl.gro" >> log_autogmx.log 2>&1

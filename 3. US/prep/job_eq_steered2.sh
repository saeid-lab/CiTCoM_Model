...
srun -K1 --mpi=cray_shasta  -N $nodes -n $ranks -c $threads  --accel-bind verbose  --cpu-bind cores   -m block:block gmx_mpi mdrun  -s $file.tpr -v -deffnm $file -plumed plumed_steered2.dat -ntomp $threads -pin on -nb gpu -bonded gpu -update cpu  -maxh 23 

...
ROOTNAME=run			# Rootname for gromacs files
MDRUN_OPT=""         		# Additional options to use in mdrun
BATCH_FNAME=REST2_restart.sh	# The file name of the submition file (this file)
...
MDRUN="srun -K1 --mpi=cray_shasta -N $nodes -n $ranks -c $threads --accel-bind verbose,closest -m block:block gmx_mpi mdrun  "
...
while [[ (! -e "0/$ROOTNAME.${prev_cycle}.cpt") && (${prev_cycle} -gt 0) ]]
do
    let prev_cycle--
done
...
#tpr_nsteps=100000000
tpr_nsteps=$(srun gmx_mpi dump -s 0/${ROOTNAME}.tpr 2> /dev/null | grep -m1 nsteps | cut -f 2 -d = | sed 's/[^0-9]//')
...
last_step=$(grep "Writing checkpoint" 0/$ROOTNAME.$prev_cycle.part*.log | tail -n1 | cut -f 4 -d ' ')
...
# anyway.
if [[ ($(ls 0/$ROOTNAME.$prev_cycle.part*.log | wc -l) -eq 0) || \
      ($last_step -lt $tpr_nsteps) ]]
then
  # Launch the parallel job
  $MDRUN -multidir {0..23} -nice 0 -s $ROOTNAME -deffnm $ROOTNAME.$cycle -plumed plumed.dat -hrex -replex 1000 -ntomp $threads -pin on -nb gpu -pme gpu -bonded gpu -update cpu -v -stepout 1000 -maxh 23 -cpi $checkpoint -noappend  $MDRUN_OPT >& $ROOTNAME.$cycle.runout

    # Check if the trajectory and the energy file are not corrupted. We will
    # not relaunch a run if a corruption occured.
    srun -n 1 gmx_mpi check -f 0/$ROOTNAME.$cycle.part*.xtc
    #gmx_mpi check -f $ROOTNAME.$cycle.part*.xtc
    check_xtc=$?
    echo "XTC check output code is $check_xtc"
    srun -n 1 gmx_mpi check -e 0/$ROOTNAME.$cycle.part*.edr
    ...
    last_step=$(grep "Writing checkpoint" 0/$ROOTNAME.$cycle.part*.log | \
            tail -n1 | cut -f 4 -d ' ')
